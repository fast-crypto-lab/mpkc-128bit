...
/// interpolation test
	...
	uint8_t d1[_PUB_N_BYTE],d2[_PUB_N_BYTE],tt[_PUB_N_BYTE],diff[_PUB_N_BYTE];
...
//#ifdef _DEBUG_
	...
	for(unsigned i=0;i<TEST_RUN;i++) {
		...
		if( ! gf256v_is_zero(tt,_PUB_M_BYTE) ) {
			...
			gf256v_fdump( stdout , d2 , _SEC_M_BYTE ); printf("\n\n");
			accu_fail ++;
#ifdef _EXT_WILE_FAIL_
			return -1;
#endif
		}
	}
	printf("debug %d/%d pk/sk test success.\n", TEST_RUN-accu_fail , TEST_RUN );
