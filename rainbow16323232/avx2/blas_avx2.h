...
void gf256mat_prod_multab_avx2( uint8_t * c , const uint8_t * matA , unsigned n_A_vec_byte , unsigned n_A_width , const uint8_t * multab ) {
	...
	uint8_t r8[96] __attribute__((aligned(32)));
...
/// slow
static inline
void gf4mat_prod_avx2( uint8_t * c , const uint8_t * matA , unsigned n_A_vec_byte , unsigned n_A_width , const uint8_t * b ) {
	assert( n_A_width <= 128 );
	...
	for(unsigned i=0;i<n_A_width;i++) {
		_r[0] = 0-(x[i]&1);
		__m256i b0 = _mm256_broadcastb_epi8( _mm_load_si128((__m128i*)_r) );
		_r[0] = 0-(x[i]>>1);
		__m256i b1 = _mm256_broadcastb_epi8( _mm_load_si128((__m128i*)_r) );

		__m256i inp = _mm256_loadu_si256( (__m256i*) matA );
		r ^= _tbl32_gf4_mul( inp , b0 , b1 );
                matA += n_A_vec_byte;
        }
