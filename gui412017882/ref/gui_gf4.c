...
void gui_pubmap_seckey( uint8_t * z , const gui_key * sk , const uint8_t * w ) {
	...
	gf4mat_prod( tt , sk->mat_s , _PUB_N_BYTE , _SEC_N , w );
	...
	gf4mat_prod( tt2 , sk->mat_t , _SEC_M_BYTE , _SEC_M , tt1 );
...
{
	gf4mat_rand_inv( pk->mat_t , sk->mat_t , _SEC_M );
	gf4mat_rand_inv( pk->mat_s , sk->mat_s , _SEC_N );
...
unsigned gui_secmap( uint8_t * w , const gui_key * sk , const uint8_t * z , const uint8_t seed[_LEN_SHA256_])
{
	...
	uint8_t temp_t[(_SEC_M_BYTE)] __attribute__((aligned(16))) = {0};
	...
	gf4mat_prod(temp_t,sk->mat_t,_SEC_M_BYTE,_PUB_M,_z);
...
///			crypto_hash_sha256( (uint8_t *)sha256_digest ,  (const uint8_t *)sha256_digest , _LEN_SHA256_ );
			...
			gf4mat_prod(temp_batch_run,(sk->mat_t)+((_PUB_M)*(_SEC_M_BYTE)),_SEC_M_BYTE,_SEC_M-_PUB_M,_z);
	...
	gf4mat_prod(w,sk->mat_s,_PUB_N_BYTE,_SEC_N,x);
//	return time;
	return (time<256)?1:0;
}
...
///	uint8_t c_eq[BGF_SIZE*C_TERMS]; };
	...
	for(unsigned i=0;i<_V;i++) vinegar[i] = gf4v_get_ele( _vinegar , i );
	uint8_t temp[BGF_SIZE] __attribute__((aligned(16)));

	const uint8_t * ptr = sk->vo[0];
	memset(temp,0,BGF_SIZE);
	for(int j=0;j<_V;j++) {
		gf16v_madd( temp , ptr , vinegar[j] , BGF_SIZE);
		ptr += BGF_SIZE;
	}
#if 0
	BGFMUL_256( poly+BGF_SIZE , poly+BGF_SIZE , temp );  /// X /// XXX:  here. warning
#else
	ISO( temp , temp );
	ISO( poly+BGF_SIZE , poly+BGF_SIZE );
	BGFMUL( poly+BGF_SIZE , poly+BGF_SIZE , temp );
	IVSISO( poly+BGF_SIZE , poly+BGF_SIZE );
#endif
	ptr = sk->vo[1];
	memset(temp,0,BGF_SIZE);
	for(int j=0;j<_V;j++) {
...
#if 0
	BGFMUL_256( poly+3*BGF_SIZE , poly+3*BGF_SIZE , temp );  /// X^4 /// XXX:  here. warning
#else
	ISO( temp , temp );
	ISO( poly+3*BGF_SIZE , poly+3*BGF_SIZE );
	BGFMUL( poly+3*BGF_SIZE , poly+3*BGF_SIZE , temp );
	IVSISO( poly+3*BGF_SIZE , poly+3*BGF_SIZE );
#endif
	ptr = sk->vo[2];
	memset(temp,0,BGF_SIZE);
	for(int j=0;j<_V;j++) {
...
#if 0
	BGFMUL_256( poly+6*BGF_SIZE , poly+6*BGF_SIZE , temp );  /// X^16 /// XXX:  here. warning
#else
	ISO( temp , temp );
	ISO( poly+6*BGF_SIZE , poly+6*BGF_SIZE );
	BGFMUL( poly+6*BGF_SIZE , poly+6*BGF_SIZE , temp );
	IVSISO( poly+6*BGF_SIZE , poly+6*BGF_SIZE );
#endif

	uint8_t vec[_V] __attribute__((aligned(16)));
	unsigned idx = 0;
	for(unsigned i=0;i<_V;i++) {
		memcpy( vec , _vinegar , (_V+3)/4 );
		gf4v_mul_scalar( vec , vinegar[i] , (_V+3)/4 );
		for( unsigned j=0;j<=i;j++) {
			gf4v_madd( poly , sk->vv[idx] , gf4v_get_ele(vec,j) , BGF_SIZE );
			idx++;
		}
	}
}
...
void _gui_cpoly_eval( uint8_t *val , const uint8_t * poly , const uint8_t * a )
{
	...
	uint8_t a16[BGF_SIZE] __attribute__((aligned(16))); BGFSQU( a16 , a8 );
	uint8_t a17[BGF_SIZE] __attribute__((aligned(16))); BGFMUL( a17 , a16 , a );
	...
	BGFMUL( temp , a4 , poly + 3*BGF_SIZE ); BGFADD( r0 , temp );
	BGFMUL( temp , a5 , poly + 4*BGF_SIZE ); BGFADD( r0 , temp );
	BGFMUL( temp , a8 , poly + 5*BGF_SIZE ); BGFADD( r0 , temp );
	BGFMUL( temp , a16 , poly + 6*BGF_SIZE ); BGFADD( r0 , temp );
	BGFMUL( temp , a17 , poly + 7*BGF_SIZE ); BGFADD( r0 , temp );
...
void gui_central_map( uint8_t * y , const gui_key * sk , const uint8_t * x ) {
	...
	ISO( _x , x );
	for(unsigned i=0;i<C_TERMS;i++) ISO( poly+i*BGF_SIZE , poly+i*BGF_SIZE );

	_gui_cpoly_eval( _y , poly , _x );
